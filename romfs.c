...
#ifdef LV_USE_FS_ROMFS && LV_USE_FS_ROMFS
#define ROMFS_LOG_ERROR	LV_LOG_ERROR
#define ROMFS_LOG_WARN	LV_LOG_WARN
#else
#define ROMFS_LOG_ERROR printf
#define ROMFS_LOG_WARN  printf
#endif
...
int romfs_init(rom_fs_t * fs)
{
    ...
    if(memcmp(&super, "-rom1fs-", 8) != 0) {
        ROMFS_LOG_ERROR("romfs_init: not a romfs file\n");
        return -1;
    }

    fs->size = __le(super.size);
    if(__checksum(fs) != 0) {
        ROMFS_LOG_ERROR("romfs_init: checksum error\n");
        return -1;
    }
                                  ...
                                  struct romfs_nodeinfo * nodeinfo)
{
    ...
    // ROMFS_LOG_ERROR("romfs_searchdir: %s not found\n", entryname);
    return -1;
}
                           ...
                           romfs_nodeinfo_t ** pnodeinfo)
{
    ...
    if(nodeinfo == NULL) {
        ROMFS_LOG_ERROR("romfs_cachenode: malloc failed for %s\n", name);
        return -ENOMEM;
    }
    ...
    do {
        ...
        if(strcmp(childname, ".") != 0 && strcmp(childname, "..") != 0) {
            if(child == NULL || nodeinfo->count == num - 1) {
                ...
                if(tmp == NULL) {
                    ROMFS_LOG_ERROR("romfs_cachenode: realloc failed for %s\n", childname);
                    return -ENOMEM;
                }
            ...
            if(ret < 0) {
                ROMFS_LOG_ERROR("romfs_cachenode: failed to cache %s\n", childname);
                nodeinfo->count--;
                return ret;
            }
        }
...
int romfs_open(rom_fs_t * fs, struct romfs_file * file, const char * path)
{
    ...
    if(fs == NULL) {
        ROMFS_LOG_ERROR("romfs_open: fs is NULL");
        return -1;
    }

    while(!cache_dentry_inited) {
        ROMFS_LOG_ERROR("romfs_open: cache not initialized");
        return -EAGAIN;
    }
    ...
    if(ret < 0) {
        // ROMFS_LOG_ERROR("romfs_open: finddirentry failed for %s\n", path);
        return ret;
    }
...
{
    if(file->offset == 0) {
        ROMFS_LOG_ERROR("romfs_close: file not opened");
        return -1;
    }
    file->curr_pos = 0;
    file->offset = 0;
...
int romfs_read(rom_fs_t * fs, struct romfs_file * file, void * buf, uint32_t nbytes)
{
    ...
    if(file->offset == 0) {
        ROMFS_LOG_ERROR("romfs_read: file not opened");
        return -1;
    }
...
static size_t file_backend_read(rom_fs_t * fs, uint32_t offset, void * buf, uint32_t size)
{
    ...
    if(fs->private_data == NULL || fd < 0) {
        ROMFS_LOG_ERROR("romfs_filebackend_read: fs or fd is invalid");
        return -1;
    }
...
rom_fs_t * romfs_filebackend_init(const char * path, uint32_t offset)
{
    ...
    if(fd < 0) {
        ROMFS_LOG_ERROR("romfs_filebackend_init: failed to open file: %s\n", path);
        return NULL;
    }

    ROMFS_LOG_WARN("romfs_filebackend_init: path: %s, offset: %lu, fd: %d\n", path, offset, fd);
    fs->start = offset;
    ...
    if(romfs_init(fs) < 0) {
        close(fd);
        free(fs);
        ROMFS_LOG_ERROR("romfs_filebackend_init: romfs_init failed\n");
        return NULL;
    }
